...
int main() {
  ...
  const double pulse_error = 0.001;  // 1 pulse error
  ...
  std::normal_distribution<double> dist_encoder(0.0, noise_encoder);
  ...
  struct State {
    ...
    double left_angular_rate;
    double right_angular_rate;
    double wi;
  };
  ...
  for (size_t index = 0; index < true_timestamp_list.size();
    ...
    const auto left_angular_rate = true_left_angular_rate_list[index];
    const auto right_angular_rate = true_right_angular_rate_list[index];
    const auto wi = true_yaw_rate_at_body_list[index];

    State state;
    state.timestamp = time;
    // state.left_angular_rate = left_angular_rate + dist_encoder(gen);
    // state.right_angular_rate = right_angular_rate + dist_encoder(gen);
    state.left_angular_rate = left_angular_rate;    //+ dist_encoder(gen);
    state.right_angular_rate = right_angular_rate;  //+ dist_encoder(gen);
  ...
  for (size_t index = 1; index < odometry_state_list.size(); ++index) {
    ...
    const double al = prev_state.left_angular_rate;
    const double ar = prev_state.right_angular_rate;
    const double vb = r / 2.0 * (al + ar);
    const double wb = r / l * (-al + ar);
    Eigen::Vector2d v_world;
    v_world.x() = vb * cos_yaw;
    v_world.y() = vb * sin_yaw;
    curr_state.pos = prev_state.pos + dt * v_world;
    curr_state.vel = v_world;
    curr_state.yaw = prev_state.yaw + dt * wb;
  ...
  R.setZero();
  R(0, 0) = noise_encoder * noise_encoder;
  R(1, 1) = noise_encoder * noise_encoder;
  Q.setIdentity();
  ...
  for (size_t index = 1; index < kf_state_list.size(); ++index) {
    ...
    const double vx_predicted = curr_state.vel.x();
    const double vy_predicted = curr_state.vel.y();
    ...
    // clang-format off
    H <<
      0.0, 0.0, 2.0 / r * cos_yaw_predicted, 2.0 / r * sin_yaw_predicted, 0.0, -l / r, //
      0.0, 0.0, 2.0 / r * cos_yaw_predicted, 2.0 / r * sin_yaw_predicted, 0.0,  l / r;
    // clang-format on

    Eigen::Vector2d z;
    z << curr_state.left_angular_rate, curr_state.right_angular_rate;
